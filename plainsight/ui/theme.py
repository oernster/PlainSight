...
library tree nor the reading pane paints one in any state.
"""
...
def stylesheet(palette: Palette, font_size: FontSize = DEFAULT_FONT_SIZE) -> str:
    ...
    A container class never appears in a ring rule. The library tree is an item
...
def document_style(palette: Palette) -> str:
    ...
    The typography is here because a document's own paragraphs can be enormous:
