...
def _reading() -> str:
    ...
    return (
        ...
        "<p>The foot of the window says what you have open and how much of "
        "it there is: the kind at the left, then the characters and the lines "
        "at the right. The count is of the text rather than of the file, so a "
        "Markdown document is counted beneath the fields it declares; a PDF or "
        "a Word document is counted as what was read out of it. A document "
        "that could not be read still names its kind, which comes from the "
        "file name, while having no count, which would need the text. "
        "Anything the window has to tell you takes the left end for a few "
        "seconds and the kind returns after it.</p>"
